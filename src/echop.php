				...
				if ($maxDepth && ($maxDepth <= $indent)) {
					...
					foreach ($propertyNames AS $eachPropertyName) {
						...
						$str.= ($phpDoc ? str_replace("\n", "\n" . str_repeat("\t", $indent+1), "\n" . $doc) : '') .
								...
								" => " .
								static::echop($val, true, $indent+1, $maxDepth, $isHtml);
					}
					$str.= "\n" . str_repeat("\t", $indent) . ')';
				}
			...
			if (empty($param)) {
				...
				if ($maxDepth && ($maxDepth<=$indent)) {
					...
					foreach ($param AS $eachKey=>$eachValue) {
						$str.= "\n" . str_repeat("\t", $indent+1) . '[' . $eachKey . "] => " . static::echop($eachValue, true, $indent+1, $maxDepth, $isHtml);
					}
					$str.= "\n" . str_repeat("\t", $indent) . ')';
				}
			}
...
function echop($param, $returnOnly=false) {
	return EchoPrinter::echop($param, $returnOnly);
}
